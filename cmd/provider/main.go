...
import (
	...
	"github.com/jwefers/provider-auth0/apis"
	"github.com/jwefers/provider-auth0/apis/v1alpha1"
	"github.com/jwefers/provider-auth0/config"
	"github.com/jwefers/provider-auth0/internal/clients"
	"github.com/jwefers/provider-auth0/internal/controller"
	"github.com/jwefers/provider-auth0/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Auth0").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-auth0"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-auth0",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Auth0 APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Auth0 controllers")
