...
type ConnectionsInitParameters struct {
	...
	// (Block Set, Min: 1) Connections that are enabled for the organization. (see below for nested schema)
	// Connections that are enabled for the organization.
	EnabledConnections []EnabledConnectionsInitParameters `json:"enabledConnections,omitempty" tf:"enabled_connections,omitempty"`

	// (String) ID of the organization on which to enable the connections.
...
type ConnectionsObservation struct {
	...
	// (Block Set, Min: 1) Connections that are enabled for the organization. (see below for nested schema)
	// Connections that are enabled for the organization.
	EnabledConnections []EnabledConnectionsObservation `json:"enabledConnections,omitempty" tf:"enabled_connections,omitempty"`
...
type ConnectionsParameters struct {
	...
	EnabledConnections []EnabledConnectionsParameters `json:"enabledConnections,omitempty" tf:"enabled_connections,omitempty"`
...
type EnabledConnectionsInitParameters struct {
...
type EnabledConnectionsObservation struct {
...
type EnabledConnectionsParameters struct {
...
// ConnectionsSpec defines the desired state of Connections
type ConnectionsSpec struct {
	...
	// autoscaler.
	InitProvider ConnectionsInitParameters `json:"initProvider,omitempty"`
}

// ConnectionsStatus defines the observed state of Connections.
type ConnectionsStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        ConnectionsObservation `json:"atProvider,omitempty"`
}
...
// Connections is the Schema for the Connectionss API. With this resource, you can manage enabled connections on an organization.
...
type Connections struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.enabledConnections) || (has(self.initProvider) && has(self.initProvider.enabledConnections))",message="spec.forProvider.enabledConnections is a required parameter"
	...
	Spec   ConnectionsSpec   `json:"spec"`
	Status ConnectionsStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// ConnectionsList contains a list of Connectionss
...
var (
	Connections_Kind             = "Connections"
	Connections_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: Connections_Kind}.String()
	Connections_KindAPIVersion   = Connections_Kind + "." + CRDGroupVersion.String()
	Connections_GroupVersionKind = CRDGroupVersion.WithKind(Connections_Kind)
)
